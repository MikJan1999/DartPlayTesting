...
import { FlexBetween, FlexCenter, FormItem } from '../../../components/layouts/Layouts';
import Chip from '../components/Chip';
import useGameOptions from './hooks/useGameOptions';

export default function GameOptions() {
    const { isValidGame, isValidPlayers, availableGames, availablePlayers, validTypes, game, players, onSubmit, onClickChip } = useGameOptions();
    ...
    return (
        <div className="animate__animated animate__fadeInLeft flex center grow panel-down">
            <div className="flex between column panel">
                <form method="GET" action="/options/players/:num/game/:game" onSubmit={onSubmit} style={{ width: '100%' }}>
                    <FormItem>
                        ...
                        <FlexCenter>
                            <div className="grid grid-two-column down small">
                                {availableGames.map(gameN => (
                                    <Chip key={gameN} value={gameN} type={validTypes.game} current={game} onClick={onClickChip}></Chip>
                                ))}
                            </div>
                        </FlexCenter>
                    </FormItem>
